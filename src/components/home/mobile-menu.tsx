...
export function MobileMenu({ links, onClose }: { links: MenuLink[]; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    media.add({ reduced: '(prefers-reduced-motion: reduce)', animated: '(prefers-reduced-motion: no-preference)' }, context => {
      ...
      const sequence = gsap.timeline()
        .fromTo(dialog, { yPercent: reduced ? 0 : -100, opacity: 1 }, {
          yPercent: 0, duration: reduced ? 0 : animations.menu.duration / 1000, ease: 'power3.out',
        })
      if (!reduced) sequence.fromTo(items, { y: 24, opacity: 0 }, {
  ...
  const close = (after?: () => void) => {
    ...
    const complete = () => { onClose(); finish.current?.() }
  ...
  return createPortal(<dialog ref={ref} id="mobile-navigation" aria-label="Mobile navigation" data-lenis-prevent
