...
export function Services() {
  ...
  return <section ref={motionRef} id="services" aria-labelledby="services-heading" className="section-shell px-page bg-surface-soft py-10 max-md:bg-white max-md:px-5 max-md:py-16">
    ...
    <div className="mt-12 max-md:mt-[22px] flex flex-col gap-1.5 max-md:gap-[22px] md:flex-row">
      {[{ title: 'Emergency Work', image: 'emergency', subject: 'Emergency work' }, { title: 'Infrastructure planning', image: 'planning', subject: 'Infrastructure planning' }].map(service => <article {...entrance('cards', service.image)} data-reveal-owner key={service.image} className={`service-card min-h-130 pt-8 px-8.5 pb-7 bg-center bg-cover [&_h3]:max-w-110.5 max-md:min-h-[310px] max-md:p-5 max-md:justify-between max-md:[&_h3]:max-w-[233px] max-md:[&_a]:w-[115px] max-md:[&_a]:min-h-[32.168px] max-md:[&_a]:h-[32.168px] max-md:[&_a]:text-[10.35px] max-md:[&_a]:gap-2 max-md:[&_a]:pl-[10.45px] max-md:[&_img]:size-[27.343px] [--reveal-delay:0] [--reveal-stagger:0] [--reveal-threshold:.01] ${service.image === 'emergency' ? "bg-[url('/services/emergency.webp')] max-md:bg-[linear-gradient(#0000004d,#0000004d),url('/services/emergency-mobile.webp')] [&_h3]:max-w-74" : "bg-[linear-gradient(#0005,#0005),url('/services/planning.webp')] max-md:bg-[linear-gradient(#0000004d,#0000004d),url('/services/planning-mobile.webp')]"} flex min-w-0 flex-1 flex-col items-start justify-end gap-7`}>
