...
export function Results() {
  ...
  return <section ref={motionRef} aria-labelledby="results-heading" className="results-section relative isolate overflow-clip min-h-194.75 pt-22.25 max-lg:min-h-0 max-lg:pt-16 [--reveal-delay:0] [--reveal-stagger:70] [--reveal-threshold:.01] flex flex-col"><div {...entrance('backdrop')} className="section-backdrop [background:linear-gradient(#0008,#6668),url('/results/background.webp')_center/cover] max-md:bg-[linear-gradient(#0008,#6668),url('/results/background-mobile.webp')] absolute inset-0 -z-10 pointer-events-none" aria-hidden="true" />
    ...
    <div className="stats-row mt-auto flex items-start max-lg:mt-12 max-lg:grid max-lg:grid-cols-2 max-lg:items-stretch max-[480px]:grid-cols-1">{stats.map((stat, index) => <article {...entrance('cards', stat.title)} data-reveal-owner className={`stat-card min-h-[12.8rem] pt-8.5 px-7.5 pb-4 bg-top-left bg-cover bg-no-repeat even:mt-[12.8rem] [&_h3]:text-base [&_.text-stat]:tabular-nums max-[1100px]:px-4 max-lg:min-h-60 max-lg:bg-surface max-lg:bg-none max-lg:even:mt-0 max-lg:p-6 [--reveal-delay:0] [--reveal-stagger:0] [--reveal-threshold:.01] ${["bg-[url('/results/card.svg')]", "bg-[url('/results/card-4.svg')]", "bg-[url('/results/card-2.svg')]", "bg-[url('/results/card-3.svg')]"][index]} flex min-w-0 flex-1 flex-col`} key={stat.title}><TextReveal as="p" className="text-stat max-lg:text-stat-mobile tracking-tight">{stat.value}</TextReveal><TextReveal as="h3" className="mt-4 text-body font-semibold uppercase">{stat.title}</TextReveal><TextReveal as="p" className="mt-4 max-w-52 text-small leading-tight">{stat.description}</TextReveal></article>)}</div>
  </section>
}
