...
export function ButtonLink({ children, href, primary = false, arrow = false, ref, 'data-entrance': entrance }: { children: ReactNode; href: string; primary?: boolean; arrow?: boolean; ref?: Ref<HTMLAnchorElement>; 'data-entrance'?: string }) {
  ...
  return href.startsWith('#')
    ? <Link ref={ref} data-entrance={entrance} to="/" hash={href.slice(1)} className={className}>{content}</Link>
    : <a ref={ref} data-entrance={entrance} href={href} className={className}>{content}</a>
}
