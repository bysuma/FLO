...
export function Navbar() {
  ...
  return <nav ref={motionRef} aria-label="Main navigation" className="navbar flex items-center gap-20 max-w-174 mx-auto pt-5.25 text-white font-display text-sm max-md:mx-5 max-md:pt-[18px] max-md:gap-4 max-md:flex-wrap [--reveal-duration:750] [--reveal-delay:80] [&_a]:transition-colors [&_a]:duration-250 [&_a:not(.brand-logo)]:motion-safe:hover:text-brand [&_a]:motion-reduce:transition-none">
    ...
    <Link {...entrance('controls', 'logo')} to="/" hash="home" aria-label="FLO Engineering home" className="brand-logo max-md:size-[69px] max-md:pb-[16.7px] max-md:bg-size-[11.87px_11.87px,contain] max-md:bg-position-[right_16px_top_28.6px,center] w-21.5 h-21.5 [background:url('/navbar/logo-detail.svg')_right_1.25rem_top_2.22rem/.925rem_.925rem_no-repeat,url('/navbar/logo.svg')_center/contain_no-repeat] flex items-end justify-center pb-[1.3rem] shrink-0"><TextReveal as="span" className="brand-word max-md:text-[3.842px] max-md:tracking-[1.806px] font-display text-[4.789px] leading-[.8] tracking-[2.2508px] text-ink">ENGINEERING</TextReveal></Link>
