...
const entrances = [{ selector: 'a, button', distance: 20 }] satisfies readonly EntranceGroup[]
...
export function Navbar() {
  ...
  return <nav ref={motionRef} aria-label="Main navigation" className="navbar flex items-center gap-20 max-w-174 mx-auto pt-5.25 text-white font-display text-sm max-md:mx-page max-md:gap-4 max-md:flex-wrap [--reveal-duration:750] [--reveal-delay:80] [&_a]:transition-colors [&_a]:duration-250 [&_a:not(.brand-logo)]:motion-safe:hover:text-brand [&_a]:motion-reduce:transition-none">
    <div className="hidden flex-1 items-center justify-between md:flex">{links.slice(0, 2).map(link => <Link key={link.href} to="/" hash={link.href.slice(1)}><TextReveal as="span">{link.label}</TextReveal></Link>)}</div>
    <Link to="/" hash="home" aria-label="FLO Engineering home" className="brand-logo w-21.5 h-21.5 [background:url('/navbar/logo-detail.svg')_right_1.25rem_top_2.22rem/.925rem_.925rem_no-repeat,url('/navbar/logo.svg')_center/contain_no-repeat] flex items-end justify-center pb-[1.3rem] shrink-0"><TextReveal as="span" className="brand-word font-display text-[4.789px] leading-[.8] tracking-[2.2508px] text-ink">ENGINEERING</TextReveal></Link>
    <div className="hidden flex-1 items-center justify-between md:flex">{links.slice(2).map(link => <Link key={link.href} to="/" hash={link.href.slice(1)}><TextReveal as="span">{link.label}</TextReveal></Link>)}</div>
    <button type="button" aria-label="Open menu" aria-haspopup="dialog" aria-expanded={open} aria-controls="mobile-navigation"
