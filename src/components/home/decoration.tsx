...
function animateDecoration(target: Element) {
  ...
  if (strokes.length) timeline.fromTo(strokes, {
    strokeDasharray: (i: number) => lengths[i],
    strokeDashoffset: (i: number) => lengths[i],
  }, {
    strokeDashoffset: 0, duration: (i: number) => durations[i], ease: 'none',
    stagger: 0, clearProps: 'strokeDasharray,strokeDashoffset',
  }, 0)
  // Solid bands share the hero mask's geometric opening.
  if (fills.length) timeline.fromTo(fills, {
    scaleY: 0, svgOrigin: (i: number) => origins[i],
  }, {
    scaleY: 1, duration: 1.45, stagger: .2,
